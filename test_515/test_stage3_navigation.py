...
def get_parser():
    ...
    parser.add_argument('--model', type=str, default='checkpoint524-stage1ablation', help='model to use')
    parser.add_argument('--json_path', type=str, default='/home/corpus/test_515/self_test/data_finetune.json', help='annotation json to use')
    parser.add_argument('--dataset', type=str, default='/home/corpus/arm_01', help='dataset to use')
    parser.add_argument('--output_dir', type=str, default='/home/corpus/test_515/result/navigation/self_navigation_stage1blation.json', help='dataset to use')
    return parser
...
if __name__ == "__main__":
    ...
    elif args.model == "checkpoint524-qwenablation":
        model_path = "/home/corpus/ckpt/qwen_stage3_QwenVL_self76K_0609/checkpoint-400"
    ...
    elif args.model == "checkpoint524-stage1ablation":
        model_path = "/home/corpus/ckpt/qwen_stage3_self76K_stage1ckpt4800_0610/checkpoint-600" 
    ...
    n = 0 
    for entry in tqdm(json_data, desc="Processing JSON entries"):
        app = entry["APP"]
        path_before = entry["path before"]
        input = entry["path after"]
        output = entry["action_dense"]
        subdir_path = os.path.join(root_dir, app)
        # print(subdir_path)
        if os.path.isdir(subdir_path):
            # print("a")
            current_image_filename = f"{path_before}-screen.png"
            current_dir_path = os.path.join(subdir_path, path_before)
            current_image_path = os.path.join(current_dir_path, current_image_filename)
            if os.path.exists(current_image_path):
                if args.model.startswith("checkpoint524") or args.model.startswith("stage3"):
                    ...
                    prompt = stage3_navigation_qwen_prompt
                    query = tokenizer.from_list_format([
                        {'text':prompt},
                        {'text':"图片："},
                        {'image': current_image_path}, # Either a local path or an url
                    ...
                    # input = f"{question}"
                    response = gpt_4v_stage3_navigation(text1, image_path1, ans1, text2, image_path3, ans2, input, current_image_path, )
