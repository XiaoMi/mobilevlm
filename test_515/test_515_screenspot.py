...
def resize_and_adjust_bbox(image, bbox, target_size=(720, 1280)):
    ...
    return resized_image, adjusted_bbox
...
def get_parser():
    ...
    parser.add_argument('--model', type=str, default='checkpoint518-6400', help='model to use')
    parser.add_argument('--dataset', type=str, default='/home/corpus/dataset/screenspot/screenspot_mobile.json', help='dataset to use')
    parser.add_argument('--output_dir', type=str, default='/home/corpus/test_515/result/grounding/mobilevlm_screenspot.json', help='dataset to use')
    return parser
...
if __name__ == "__main__":
    ...
    for data_entry in tqdm(test_data, desc="Processing test data"):
        img_filename = data_entry['img_filename']
        print(img_filename)
        bbox = data_entry['bbox']
        print(bbox)
        instruction = data_entry['instruction']

        current_image_path = os.path.join('/home/corpus/dataset/screenspot', img_filename)  # Update with the correct image directory

        # Load and resize the image
        image = Image.open(current_image_path).convert('RGB')
        resized_image, adjusted_bbox = resize_and_adjust_bbox(image, bbox, target_size=(720, 1280))
        

        # Save the resized image for visualization/debugging if needed
        resized_image_path = os.path.join('/home/corpus/dataset/screenspot/resized_images', img_filename)
        resized_image.save(resized_image_path)
        if args.model.startswith("checkpoint518"):
            query = tokenizer.from_list_format([ 
                {'image': resized_image_path},
                {'text': f"告诉我{instruction}的位置"},
            ])
            ...
            if match1:
                ...
                x1_pred, y1_pred = map(int, bounds1[0].split(','))
                x2_pred, y2_pred = map(int, bounds1[1].split(','))
                result = calculate_iou(x1_pred, y1_pred, x2_pred, y2_pred, bbox)
            else: 
                result = 0
                print("未匹配成功！")
        ...
        while match1 is None and count < 3 and args.model != 'gpt-4v' and args.model != 'qwen-vl-plus' and args.model != 'qwen-vl-max': 
            ...
            print("ground truth:", adjusted_bbox)
            ...
            if match1:
                ...
                result = calculate_iou(x1_pred, y1_pred, x2_pred, y2_pred, adjusted_bbox)
            else:
                match1 = re.search(r'\(([0-9]+,[0-9]+)\),\(([0-9]+,[0-9]+)\)', response)
                if match1:
                    bounds1 = [match1.group(1), match1.group(2)]
        ...
        data = {
            "subpath": img_filename,
            "predict box" : response,
            "target box" : adjusted_bbox,
            "aveIOU" : acc
        }
