...
def calculate_scroll_parameters(bounds, direction):
...
def generate_task1_input(instruction=random.choice(page_structure_instruction)):
    # 任务1 描述页面结构
    return instruction
...
def generate_task4_input(instruction=random.choice(page_description_instruction)):
    """
       4. 屏幕总结-整体(Caption)
       """
    return f"\n{instruction}"
...
def generate_task5_input(instruction=random.choice(next_page_action_instruction)):
    """
       5. 屏幕跳转action生成 （Two Image Navigation）
       """
    return f"\n{instruction}"

def generate_task2_input(instruction=random.choice(page_action_space_instruction)):
    """
      2. 屏幕action space生成
       """
    return f"\n{instruction}"
...
if __name__ == "__main__":
    ...
    # print(print_tree_with_text_and_bounds(anytree_root))
