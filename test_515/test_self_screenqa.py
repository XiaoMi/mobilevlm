...
def get_parser():
    ...
    parser.add_argument('--dataset', type=str, default='/home/corpus/arm_01/ctrip_graph_8100', help='dataset to use')
    parser.add_argument('--annotation', type=str, default='/home/corpus/test_515/self_test/sample_20000_finetune_task-screenqa.csv', help='test json to use')
    parser.add_argument('--output_dir', type=str, default='/home/corpus/test_515/result/screenqa/self_qwenvlmax.json', help='dataset to use')
    return parser
...
if __name__ == "__main__":
    ...
    model_path = None
    if args.model == "checkpoint531-7000":
    ...
    elif args.model == "checkpoint531-400":
        model_path = "/home/corpus/ckpt/qwen_stage3_Screen_0531/checkpoint-400"
    ...
    with open(args.annotation, 'r', encoding='utf-8') as f:
        ...
        fieldnames = annotations.fieldnames  # 获取列名
        print("列名:", fieldnames)  # 输出列名以检查
        for annotation in tqdm(annotations, desc="Processing annotations"):
            directory = annotation['\ufeff编号']
            question = annotation['问题']
            answer = annotation['答案']
            image = annotation['图像名（没找到就跳过）']
            ...
            image_dir = os.path.join(root_dir, directory)
            image_path = os.path.join(image_dir, f"{directory}-screen.png")
            
            if os.path.exists(image_path):
                if args.model.startswith("checkpoint531"): 
                    ...
                    response = dashscope.MultiModalConversation.call(model='qwen-vl-plus',
                                                     messages=messages)
                    try:
                        predicted_answer = response['output']['choices'][0]['message']['content'][0]['text']
                ...
                best_f1 = compute_f1(predicted_answer, answer)
                total_f1 += best_f1
