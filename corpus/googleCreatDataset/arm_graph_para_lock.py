...
def get_parser():
    ...
    parser.add_argument('--appid', type=int, default=8201, help='appium client port')
    ...
    parser.add_argument('--noneprefix', type=str, default='0')
    parser.add_argument('--resave', type=str, default='0')
    parser.add_argument('--startpage', type=str, default='0')
    return parser
...
def check_index(index,all_unique_page,max_steps,data_all2unique,lock_prefix,none_prefix):
    flag=True
    ...
    if lock_prefix!="0":
        ...
        prefix_list=lock_prefix.split("###")
        for temp_prefix in prefix_list:
            if name_before.startswith(temp_prefix):
                prefix_flag=True
        if prefix_flag==False:
            return False
        if none_prefix!="0":
            none_str_list=none_prefix.split(",")
            for none_str in none_str_list:
                str1="_"+str(none_str)
                str2=str1+"_"
                if str2 in name_before or name_before.endswith(str1):
                    return False
...
def capture_screenshot(device_name, data_output_path, temp_save_path, filename):
    # 在设备上保存截图
    remote_path = f"/sdcard/{filename}-screen.png"
    screencap_command = f"adb -s {device_name} shell screencap -p {remote_path}"
    subprocess.run(screencap_command, shell=True)
    # time.sleep(1)

    # 将截图文件拉取到本地
    local_path = os.path.join(data_output_path, temp_save_path, f"{filename}-screen.png")
    pull_command = f"adb -s {device_name} pull {remote_path} {local_path}"
    subprocess.run(pull_command, shell=True)
    # time.sleep(1)
    # 删除设备上的截图文件（可选）
    delete_command = f"adb -s {device_name} shell rm {remote_path}"
    subprocess.run(delete_command, shell=True)
    # time.sleep(1)

def save_input(name_before,data_output_path,start_APP_command,driver,device_name,waitadb,name_en,all_action_id):
        history_actions=[]
        temp_actions=restart_from_homepage(start_APP_command,history_actions,driver,device_name,waitadb,name_en)
        temp_save_path=name_before[:]
        ...
        for i in range(len(levellist)):
            ...
            capture_screenshot(device_name, data_output_path, temp_save_path, page2)
...
def lock_set_hold(name_before,all_data_index,data_output_path,name_before_curr):
    # 使用示例
    # lock = FileLock(data_output_path+"all_unique_page.json")
    # lock.lock()
...
def lock_set_not_done(startPage,data_output_path):
    # 使用示例
    # lock = FileLock(data_output_path+"all_unique_page.json")
    # lock.lock()
    ...
    for index in range(len(all_unique_page["data"])):
        if all_unique_page["data"][index]["name"]==startPage:
            all_unique_page["data"][index]["hold"]=False
            all_unique_page["data"][index]["done"]=False
            print("index",index)
...
def lock_set_recheck(data_output_path):
    # 使用示例
    # lock = FileLock(data_output_path+"all_unique_page.json")
    # lock.lock()
...
def lock_set_done(data_output_path,all_data_index):
    # 使用示例
    # lock = FileLock(data_output_path+"all_unique_page.json")
    # lock.lock()
...
def lock_all_action_id(action_res,data_output_path):
    # 使用示例
    # lock = FileLock(data_output_path+"all_action_id.json")
    # lock.lock()

    all_action_id=load_dict(data_output_path+"all_action_id.json")
    ...
    return all_action_id


def lock_all_page_actions(all_data_index,action_valid_flag,action_index):
    # 使用示例
    # lock = FileLock(data_output_path+"all_page_actions.json")
    # lock.lock()

    all_page_actions=load_json(data_output_path+"all_page_actions.json")#name, [action1,action2]

    all_page_actions["data"][all_data_index][action_valid_flag].append(action_index)
...
def resave_path(data_output_path,name_before,start_APP_command,driver,device_name,waitadb,name_en):

    os.makedirs(data_output_path+name_before+"/",mode=511,exist_ok=True)
    history_actions=[]
    temp_actions=restart_from_homepage(start_APP_command,history_actions,driver,device_name,waitadb,name_en)
    ...
    for i in range(len(levellist)):
        ...
        window_size = driver.get_window_size()
        action_list_before= gen_action_list(action_space,window_size,mapping)
        capture_screenshot(device_name, data_output_path, name_before , page2)
        # screencap_command = f"adb -s {device_name} shell screencap -p >"+data_output_path+name_before+"/"+page2+"-screen.png"
        # subprocess.run(screencap_command,shell=True)

        save_txt(data_output_path+name_before+"/"+page2+"-html.txt",html_code_before)
        save_txt(data_output_path+name_before+"/"+page2+"-xml.txt",xml_code_before)
        temp_valid=False
        ...
        data_json_before={"round":i+1, "name":page2,"unique_name":data_all2unique[page2],"valid":temp_valid,\
            "start_APP":start_APP_command,"history_actions":his_list,\
            "action_space":action_list_before,"path":path_list}
        save_json(data_output_path+name_before+"/"+page2+".json",data_json_before)

    return 



def lock_new_page(data_output_path,name_before,name_after,bm25,diff_max,diff_png,appPackage,html_code_after,action_index,all_data_index):


    # lock1 = FileLock(data_output_path+"all_unique_page.json")
    # lock1.lock()
    # lock2 = FileLock(data_output_path+"all_page_actions.json")
    # lock2.lock()
    # lock3 = FileLock(data_output_path+"all_page_id.json")
    # lock3.lock()
    # lock4 = FileLock(data_output_path+"all_triple.json")
    # lock4.lock()
    # lock5 = FileLock(data_output_path+"data_unique2all.json")
    # lock5.lock()
    # lock6 = FileLock(data_output_path+"data_all2unique.json")
    # lock6.lock()
    # print("lock")
    ...
    if name_after not in all_page_id:
        ...
        has_name_after=False
        for i in range(len(all_unique_page["data"])):
            temp_data=all_unique_page["data"][i]["name"]
            if temp_data==name_after:
                has_name_after=True
        if has_name_after==False:
        ...
        for i in range(len(all_page_actions["data"])):
            temp_data=all_page_actions["data"][i]["name"]
            if temp_data==name_after:
                has_name_after=True
        if has_name_after==False:
    ...
    return bm25,all_unique_page,all_triple,all_page_actions,all_page_id,data_unique2all,data_all2unique




def lock_path_exist(data_output_path,name_before,name_after,bm25,diff_max,diff_png,action_res,action_index,all_data_index):
...
def lock_new_path(data_output_path,name_before,name_after,bm25,diff_max,diff_png,appPackage,html_code_after,action_index):
...
def lock_load_all(data_output_path):
    ...
    # lock6 = FileLock(data_output_path+"data_all2unique.json")
    # lock6.lock()
    # lock7 = FileLock(data_output_path+"all_action_id.json")
    # lock7.lock()
    # print("lock")
    ...
    try:
        ...
        #all_page_actions=load_json(data_output_path+"all_page_actions_temp.json")
        all_page_actions={"num":0, "data":[]}
        for i in range(len(all_unique_page["data"])):
            if i%1000==0:
                print(i)
            assert all_unique_page["data"][i]["name_id"]== i
            temp_data_name=all_unique_page["data"][i]["name"]
            temp_action_valid=[]
            prefix=temp_data_name+"_"
            for key in all_page_id:
                if key.startswith(prefix):
                    new_valid=key.split(prefix)[1].split("_")[0]
                    int_valid=int(new_valid)
                    if int_valid not in temp_action_valid:
                        temp_action_valid.append(int_valid)

            temp_data={"name":temp_data_name,"action_valid":temp_action_valid,"action_invalid":[]}
            all_page_actions["data"].append(temp_data)
        all_page_actions["num"]=len(all_page_actions["data"])
        save_json(data_output_path+"all_page_actions.json",all_page_actions)

    '''
    if "xiaomiShop_graph_8158" in data_output_path:
        i=0
        while i < len(all_unique_page["data"]):
            if i != all_unique_page["data"][i]["name_id"]:
                print("##################")
                print(i)
                print(all_unique_page["data"][i])
                all_unique_page["data"].pop(i-1)
                all_unique_page["need"].pop(i-1)
                if i != all_unique_page["data"][i]["name_id"]:
                    print(i)
                    print(all_unique_page["data"][i])
                    assert all_unique_page["data"][i]["name_id"]== i
        ...
        for i in range(len(all_unique_page["data"])):
            while all_page_actions["data"][i]["name"] != all_unique_page["data"][i]["name"]:
                all_page_actions["data"].pop(i)
        for i in range(len(all_unique_page["data"])):
            name_=all_unique_page["data"][i]["name"]
            if all_page_id[name_]!=all_unique_page["data"][i]["name_id"]:
                all_page_id[name_]=all_unique_page["data"][i]["name_id"]

        for i in range(len(all_unique_page["data"])):
            assert all_unique_page["data"][i]["name_id"]== i
            assert all_page_actions["data"][i]["name"]== all_unique_page["data"][i]["name"]
            assert all_page_id[all_unique_page["data"][i]["name"]]==all_unique_page["data"][i]["name_id"]
        save_json(data_output_path+"all_page_actions.json",all_page_actions)
        save_json(data_output_path+"all_unique_page.json",all_unique_page)
        save_dict(data_output_path+"all_page_id.json",all_page_id)
    '''
    ...
    return all_unique_page,all_triple,all_page_actions,all_page_id,data_unique2all,data_all2unique,all_action_id
...
if __name__ == '__main__':
    ...
    # read data
    with open("corpus/human_anno_data_tobe.json", 'r', encoding="utf-8") as file:
        data = json.load(file)
    ...
    lock_prefix=args.prefix
    none_prefix=args.noneprefix

    if name_en in app_package_data:
        name_ch=app_name_en_data[name_en]["name_ch"]
    else:
        name_ch=name_en
    data_output_path="corpus/arm_01/"+name_en+"_graph_"+str(appId)+"/"
    ...
    data_restart_path="corpus/random_walk/"+name_en+"_graph_"+str(appId)+"/"

    skip_keyword_list=["长辈模式","跟随系统","夜间","深色","上次网页","模式", "皮肤","skin manage","撤销授权","上车即连","青少年模式","全健康内容家长放心","儿童模式","专属卡通体验","专为长辈设计","退出账号","退出当前账号","退出登录","上次未关闭网页"]
    skip_keyword_list.append("语言")
    skip_keyword_list.append("退出登录")
    skip_keyword_list.append("小米科技园")
    skip_keyword_list.append("星专送")
    skip_keyword_list.append("生活馆")
    skip_keyword_list.append("省心购")
    skip_keyword_list.append("团餐")
    skip_keyword_list.append("送心意")
    skip_keyword_list.append("拼单")
    skip_keyword_list.append("星会员")
    skip_keyword_list.append("预约")
    skip_keyword_list.append("我的")
    skip_keyword_list.append("首页")
    skip_keyword_list.append("礼卷")
    skip_keyword_list.append("1.4")
    skip_keyword_list.append("抽奖")
    skip_keyword_list.append("有星人")
    skip_keyword_list.append("礼卷")
    if name_en=="QQ":
        ...
        skip_keyword_list.append("语言")
        skip_keyword_list.append("退出登录")
    elif name_en =="starbucksen":
        skip_keyword_list.append("Language")
        skip_keyword_list.append("Sign out")
    elif name_en == "heyteago":
        skip_keyword_list.append("注销账号")
        skip_keyword_list.append("退出当前登录")
    ...
    if not os.path.exists(data_output_path+name_en+"0/"):
        ...
        capture_screenshot(device_name, data_output_path, curr_path , curr_path)
    ...
    all_unique_page,all_triple,all_page_actions,all_page_id,data_unique2all,data_all2unique,all_action_id=lock_load_all(data_output_path)
    docs=[]
    docs_names=[]
    index=0
    print("here")
    print("data_output_path",data_output_path)
    print("all_unique_page",len(all_unique_page["data"]))
    #if False: 


    if args.resave!="0":
        resave_list=resave.split(",")
        for resave_ in resave_list:
            print("###############################")
            print("resave:",resave_)
            resave_path(data_output_path,resave_,start_APP_command,driver,device_name,waitadb,name_en)
    ...
    for i in range(len(all_unique_page["data"])):
        try:
            ...
            print(i)
            print(all_unique_page["data"][i]["name_id"])
    ...
    if recheck>0:
        for i in range(len(all_unique_page["data"])):
            if lock_prefix!="0":
                prefix_list=lock_prefix.split("###")
                name_before=all_unique_page["data"][i]["name"]
                        ...
                        if none_prefix!="0":
                            ...
                            for none_str in none_str_list:
                                ...
                                if str2 in name_before or name_before.endswith(str1):
                                    ...
                                    break
                        if prefix_flag==False:
                            temp_name_before=name_before.split(temp_prefix,1)[1].split("_")
                            if len(temp_name_before)<=2:
                                print("recheck path:", name_before)
                                all_unique_page["data"][i]["hold"]=False
                                all_unique_page["data"][i]["done"]=False
                                #add_input(all_unique_page["data"][i]["name"],data_output_path)
                                save_input(all_unique_page["data"][i]["name"],data_output_path,start_APP_command,driver,device_name,\
                                    waitadb,name_en,all_action_id)
                        break
    '''
    if recheck!=0:
        all_unique_page=lock_set_recheck(data_output_path)
    ...
    if startPage != "0":
        print("here")
        all_unique_page=lock_set_not_done(startPage,data_output_path)

    none_str_list=none_prefix.split(",")
    print("none_prefix,none_str_list")
    print(none_prefix)
    print(none_str_list)
    ...
    while len(all_unique_page["need"])>0:
        if startPage != "0":
            ...
            for index in range(len(all_unique_page["data"])):
                if all_unique_page["data"][index]["name"]==startPage:
                    ...
                    if all_unique_page["data"][index]["hold"]==False and all_unique_page["data"][index]["done"]==False:
                        random_index=index
                        index_flag=True
                    print("hold",all_unique_page["data"][index]["hold"])
                    print("done",all_unique_page["data"][index]["done"])
                    print(index)
                    print(startPage)
            ...
            if index_flag==False:
                startPage="0"
        if startPage =="0":
            index_flag=check_index(random_index,all_unique_page,max_steps,data_all2unique,lock_prefix,none_prefix)
            if index_flag==False:
            ...
            if index_flag==False:
                ...
                for i in range(1,max_len):
                    ...
                    index_flag=check_index(new_index,all_unique_page,max_steps,data_all2unique,lock_prefix,none_prefix) 
                    if index_flag==True:
                        temp_flag=1
                        break
                if temp_flag==0:
        ...
        temp_save_path="temp_save"+str(systemPort)
        ...
        for i in range(len(levellist)):
            ...
            if appPackage not in html_code_before: 
                ...
                print("html_code_before",html_code_before)
        ...
        if skipFlag==True:
            all_unique_page=lock_set_hold(name_before,all_data_index,data_output_path,name_before_curr)
            #all_unique_page=set_hold(name_before,all_data_index,all_unique_page)
        ...
        if skipFlag==False:
            ...
            window_size = driver.get_window_size()
            action_list_save = gen_action_list(action_space,window_size,mapping)
            #print("action_list_save", len(action_list_save))
            #print("name_before",name_before)
            #print(action_list_save)
            ...
            for action_res in action_list_before:
                ...
                if action_res in all_action_id:
                    ...
                    #action_index=len(all_action_id)
                    #all_action_id[action_res]=action_index
                    all_action_id=lock_all_action_id(action_res,data_output_path)
                    action_index=all_action_id[action_res]
                    name_after=name_before+"_"+str(action_index)
                ...
                if os.path.exists(data_output_path+name_after+"/"+name_after+".json"): #already have page
                    if name_after not in all_page_id:
                        bm25,all_unique_page,all_triple,all_page_actions,all_page_id,data_unique2all,data_all2unique=lock_path_exist(data_output_path,\
                            name_before,name_after,bm25,diff_max,diff_png,action_res,action_index,all_data_index)
                ...
                if "[Fail]:" in last_action_result or xml_source==None or skipFlag ==True:
                    if action_index not in all_page_actions["data"][all_data_index]:
                        ...
                        all_page_actions=lock_all_page_actions(all_data_index,"action_invalid",action_index)
                    #shutil.rmtree(data_output_path+page_name+"/")
                    action_fail_Flag=True
                    #print("last_action_result",last_action_result)
                    #print("xml_source",xml_source)
                    #print("skipFlag",skipFlag)
                    break
                ...
                capture_screenshot(device_name, data_output_path, temp_save_path , name_after)
                ...
                bm25,all_unique_page,all_triple,all_page_actions,all_page_id,data_unique2all,data_all2unique=lock_new_page(data_output_path,\
                    name_before,name_after,bm25,diff_max,diff_png,appPackage,html_code_after,action_index,all_data_index)
            ...
            if action_after_Flag==False: 
                all_unique_page=lock_set_done(data_output_path,all_data_index)
                #all_unique_page["data"][all_data_index]["done"]=True
            #else:
